...
router.post('/upload', auth, (req, res, next) => {
  ...
  try {
    ...
    // Spawn Python parser in background. Whitelist the format value before
    // passing it to the Python subprocess — spawn() with array args isn't
    // shell-injection vulnerable, but an unbounded format string from the
    // request body could probe parser internals or cause undefined behavior.
    // Allowed values mirror the parser's own supported formats
    // (see parse_cycling.py module docstring).
    const VALID_FORMATS = ['generic', 'elitech', 'neware', 'arbin', 'biologic'];
    const format = VALID_FORMATS.includes(equipment_type) ? equipment_type : 'generic';
    processFile(sessionId, rawPath, format);
