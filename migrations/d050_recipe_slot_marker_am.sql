...
-- from the bare "x" to "АМ" (активный материал).
--
...
-- material goes here"; "АМ" is the lab's own shorthand and is instantly
-- readable ("96 АМ : 2.2 Super P : 1.8 PVDF"). Cosmetic only — no
...
UPDATE tape_recipes
SET name = regexp_replace(name, '^([0-9]+([.,][0-9]+)? )x( :)', '\1АМ\3')
...
BEGIN
  IF to_regclass('public.schema_migrations') IS NOT NULL THEN
    ...
    VALUES ('d050_recipe_slot_marker_am.sql', 'dalia', 'migration_file',
            'Recipe active-slot marker renamed from "x" to "АМ" in composition-derived names.')
