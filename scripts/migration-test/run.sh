#!/usr/bin/env bash
# ============================================================================
# Migration rehearsal — proves d041 … d050 apply cleanly on top of a
...
UPGRADE=(
  d041_project_participants d042_project_leads_as_team_members
  d043_enable_multi_battery_electrode_sources
  d044_access_level_none d045_material_instance_components_ddl
  ...
  d052_materials_manufacturer_families d053_tape_and_batch_files
)
...
echo "### 2. Apply the upgrade under test: d041 ... d053"
for m in "${UPGRADE[@]}"; do
  if apply "$TEST_DB" "$m"; then echo "  applied $m"; else echo "  [FAIL] $m errored"; FAILS=$((FAILS+1)); fi
done

echo "### 3. Verify ledger + data integrity"
check "ledger dalia (28->41)" "$(q "$TEST_DB" "SELECT count(*) FROM schema_migrations WHERE migration_stream='dalia'")" 41
...
echo "### 5. Idempotency — re-apply d041..d053, expect clean no-op"
...
check "ledger still 41 dalia after re-run" "$(q "$TEST_DB" "SELECT count(*) FROM schema_migrations WHERE migration_stream='dalia'")" 41
