...
App-side note (2026-07-17, updated 2026-07-29 — `dev:bypass` no longer
exists): the `dev` npm script uses a POSIX `PORT=3003` prefix and will not
run in cmd.exe/PowerShell. The lab
...
- Current local `badb_app_v1` has `public.schema_migrations` counts of
  `dima = 21` and `dalia = 28` after `d040` is applied.
...
psql -U $env:DB_USER -d $env:DB_NAME -v ON_ERROR_STOP=1 -f migrations_ASCII/d040_add_coated_thickness_fields.sql
```
...
After `d032` through `d040` succeed:
