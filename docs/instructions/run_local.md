...
- The target database must exist and be migrated through the current required
  ...
  stream through `d037_add_viscosity_conditions.sql`.
- `public.schema_migrations` is the authoritative ledger for applied migration
  state. Current local `badb_app_v1` reports `dima = 21` and `dalia = 25`
  after `d037` is applied.
...
Before Windows/lab pilot use, apply the current catch-up set through `d037` and
