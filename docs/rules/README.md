# Rules

Created: 2026-05-06
Edited: 2026-05-06
Status: rule

This folder is for hard rules that agents and colleagues must follow.

Small wording clarifications are fine. Behavior-changing rule edits need explicit approval or a clear user request.

## Rule Docs

- [AI Rules From Dalia](ai_rules_from_dalia.md)
- [Auth Policy](auth_policy.md)
- [Battery Lifecycle Rules](battery_lifecycle_rules.md)
- [Electrode Stack Rules](electrode_stack_rules.md)
- [Material Composition Rules](material_composition_rules.md)
- [Schema And API Naming Rules](schema_api_naming.md)
