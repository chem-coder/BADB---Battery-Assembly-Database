...
| Количество миграций | 43 SQL-файла в `migrations/` и 43 SQL-файла в `migrations_ASCII/`: базовый диапазон `001`–`020` плюс Dalia-миграции `d013`–`d034` |
| Authoritative migration ledger | `public.schema_migrations` (`dima = 21`, `dalia = 22` после `d034`); flat `migrations_log.txt` — human checkpoint notes only |
