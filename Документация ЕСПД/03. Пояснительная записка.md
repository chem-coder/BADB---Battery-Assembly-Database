...
3. **Интеграция с существующими инструментами.** Значительная часть рабочих процессов лабораторий организована через Excel-шаблоны. Собственная разработка позволяет сохранить привычные инструменты операторов (Excel-клиент `DatabaseUI.xlam`) и интегрировать их с новой серверной частью.
...
- сохранит возможность работы привычными инструментами (Excel) через API;
...
| Одностраничное приложение (SPA) | Vue 3 + REST API | Принято на текущем этапе. Миграция на Vue SPA завершается в апреле 2026 (фаза δ). |
...
```mermaid
flowchart TD
    Client["Клиентская часть<br/>(Vue SPA + статический HTML)"]
...
### 3.3.2. Клиентская часть — Vue 3 + Vite + Pinia

**Рассмотренные альтернативы:** React, Angular, Svelte, продолжение работы со статическими HTML-страницами.

**Принятое решение:** Vue 3.

**Обоснование:**

- Порог входа ниже, чем у React и Angular; близок к статическому HTML с добавлением реактивности.
- Однофайловые компоненты (`.vue`) объединяют шаблон, логику и стили в одном файле, что соответствует принципу колокации.
- Vite как сборщик обеспечивает быстрый dev-цикл за счёт ESM-модулей без промежуточной сборки.
- Pinia — официально рекомендуемое хранилище состояния для Vue 3; заметно проще в использовании, чем Vuex.
- PrimeVue как библиотека готовых UI-компонентов снижает трудозатраты на реализацию типовых элементов интерфейса (таблицы, формы, диалоги).
...
| 7 | Рабочий процесс — модули | Сборка модулей из батарей *(в разработке)* |
...
**Ключевые правила** (первоисточник — `materials-inventory-context.md`, `Material Sources Information.md`):
...
- `active_materials` — таблица, связывающая экземпляр материала (`material_instance_id`) с его ролью (`role`: `cathode` / `anode`) и теоретическими показателями ёмкости (`th_capacity_mah`, `th_capacity_ma_g`). Создана до введения `material_properties` (миграция d027). По мере миграции логики расчёта ёмкости на `material_properties.specific_capacity_mAh_g` данная таблица может быть деактивирована. @уточнить окончательное соотношение ролей обеих таблиц.
...
- Таблица `electrode_cut_batch_projects` — связь «многие ко многим» между партиями нарезки электродов и проектами (миграция d029). При создании партии в `electrode_cut_batch_projects` копируются все проекты родительской ленты из `tape_projects` плюс проект из `electrode_cut_batches.project_id` (если задан напрямую).
...
erDiagram
    ...
    battery_coin_config {
        ...
        varchar coin_cell_mode "half|full"
...
**Ключевые правила** (первоисточник — `electrodes-logic.md`):

Поле `batteries.form_factor` имеет CHECK-ограничение на значения `coin | pouch | cylindrical`. Распознавание полуячейки производится через подтаблицу `battery_coin_config.coin_cell_mode` (`half` | `full`).
...
|---|---|---|---|---|
| Монета половинная | `coin` | `half` | Ровно один (или катод, или анод) | Один электрод |
| Монета полная | `coin` | `full` | Ровно один катод + один анод | Один катод + один анод |
| Pouch | `pouch` | — | Ровно один катод + один анод | `катоды = аноды` или отличие на 1 |
| Цилиндрическая | `cylindrical` | — | Ровно один катод + один анод | `катоды = аноды` или отличие на 1 |
...
*По состоянию на 22.04.2026 — в разработке. Используется страница-заглушка `PlaceholderPage.vue`. Полная реализация не входит в область настоящей документации.*
...
На текущий момент (30.04.2026) база данных содержит:

| Показатель | Значение |
|---|---|
| Количество таблиц | ≈ 61 |
| Количество типов-перечислений (ENUM) | ≥ 7 |
| Количество внешних ключей | > 100 |
| Количество миграций | 36 (21 основная `001`–`020` + 1 параллельная `008`, 15 дополнительных `d013`–`d027`) |
...
- Документация Node.js 20 (https://nodejs.org/dist/latest-v20.x/docs/api/).
...
Первоисточники (материалы Д.К. Мараулайте), использованные при формировании настоящей записки:

- `materials-inventory-context.md` — концепция учёта материалов, партий поставки, свойств.
- `Material Sources Information.md` — детализированная модель партий поставки и системы оценки качества.
- `capacity-calculations-context.md` — научная логика расчёта ёмкости.
- `capacity-calculations-implementation-plan.md` — поэтапный план реализации расчёта.
- `density-calculation-context.md` — логика конвертации масса ↔ объём.
- `electrodes-logic.md` — правила форм-факторов и состава стека.
- `Sidedness.md`, `2-sided_tapes_steps.md`, `Fix coating step.md` — логика типа нанесения.
- `BADB_APP_ARCHITECTURE.md` — общее описание архитектуры.
- `BADB_APP_PROJECT_MAP.md` — структура проекта.
- `LAUNCHING_BADB_MAIN.md` — процедуры запуска.
...
| `battery_electrochem` | Файлы электрохимических испытаний |

### Домен «Рабочий процесс — модули» (в разработке)
