---
title: "Пояснительная записка: Протоколы сборки лаборатории ХИТ"
...
Разработка программного обеспечения **«Протоколы сборки лаборатории ХИТ»** (далее — **Решение**; техническое обозначение в исходном коде и БД — **BADB**, *Battery Assembly Database*) ведётся Центром химических источников тока ООО «РЭНЕРА» (ГК «Росатом») с 28 января 2026 года.
...
Программа относится к классу **лабораторных информационных систем** (ЛИС/ЛИУС; англ. LIMS) с научной специализацией на области R&D химических источников тока (разработка электродных материалов, электролитов, сепараторов, ячеек и модулей различных электрохимических систем). По сравнению с универсальными LIMS-решениями (LabWare, STARLIMS и подобные), собственная разработка оправдана следующими факторами:
...
3. **Интеграция с существующими источниками данных.** Значительная часть исторических рабочих процессов лабораторий была организована через Excel-шаблоны. Собственная разработка сохраняет возможность приёма данных от внешних JSON-источников через `/api/submit`; старый Excel/VBA-клиент `DatabaseUI.xlam` сохранён как исторический материал и не является текущим интерфейсом пилотной эксплуатации.
...
До начала разработки решения учёт лабораторных данных в Центре химических источников тока вёлся следующими средствами:
...
| Статический vanilla web interface + REST API | HTML/JS pages under `public/` | Принято как текущий рабочий интерфейс vanilla v1 для пилотной эксплуатации. |
| Одностраничное приложение (SPA) | Vue 3 + REST API | Сохранено в репозитории как отдельная поверхность следующего интерфейса; не определяет текущее поведение рабочей версии при расхождениях. |
...
```mermaid
flowchart TD
    Client["Клиентская часть<br/>(vanilla v1 public HTML/JS + Vue/V2-интерфейс синхронизации)"]
...
### 3.3.2. Клиентская часть — vanilla v1 + следующий Vue-интерфейс

**Рассмотренные альтернативы:** продолжение работы со статическими HTML-страницами, Vue 3 SPA, React, Angular, Svelte.

**Принятое текущее решение для пилотной эксплуатации:** vanilla v1 в `public/` является рабочим интерфейсом, описываемым в текущих разделах ЕСПД. Этот выбор оправдан тем, что именно vanilla-страницы сейчас содержат проверенное поведение для рабочих и справочных страниц: фильтры списков, закреплённые шапки открытых записей, текущие печатные отчёты, привязанные файлы, производный статус батарей и управляемые критичные сценарии удаления/разборки.

**Vue 3 + Vite + Pinia:** параллельная поверхность следующего интерфейса сохранена в `client-web/` и должна синхронизироваться с vanilla v1 по `docs/instructions/frontend_parity_handoff.md`. Её преимущества (компонентная модель, Vite dev loop, Pinia, PrimeVue) остаются релевантными для дальнейшей разработки, но текущая формальная документация не должна описывать Vue-расхождения как реализованное поведение рабочей версии.
...
erDiagram
    ...
    tape_step_mixing {
        ...
        text viscosity_conditions
    }
    tape_step_coating {
        int step_id PK,FK
        int coating_method_id FK
        decimal gap_um
        decimal gap_um_side2
        decimal coated_thickness_um
        decimal coated_thickness_um_side2
        decimal speed_m_min
...
- В `tape_step_coating` зазор нанесения (`gap_um`, `gap_um_side2`) и измеренная толщина после нанесения/сушки до каландрирования (`coated_thickness_um`, `coated_thickness_um_side2`) являются разными величинами. Поля стороны 2 используются только при `coating_sidedness = 'two_sided'`. Толщины стадии каландрирования хранятся отдельно в `tape_step_calendering`.
- `item_created_at` — пользовательская физическая дата создания ленты; по умолчанию текущая дата, редактируется только в прошлое/сегодня. Системное `created_at` остаётся audit timestamp создания записи в БД.
...
erDiagram
    electrode_cut_batches {
        ...
        decimal length_mm
        decimal width_mm
        text target_form_factor "coin|pouch|prism|cylindrical"
...
- `electrode_cut_batches.item_created_at` — пользовательская физическая дата создания партии электродов; `created_at` остаётся системной датой создания записи.
...
erDiagram
    batteries {
        ...
        int project_id FK
        int created_by FK
        text form_factor "coin|pouch|prism|cylindrical"
...
`batteries.item_created_at` хранит пользовательскую физическую дату создания батареи; `batteries.created_at` остаётся системным timestamp создания записи.

Поле `batteries.form_factor` имеет CHECK-ограничение на значения `coin | pouch | prism | cylindrical`. Распознавание полуячейки производится через подтаблицу `battery_coin_config.coin_cell_mode` (`half_cell` | `full_cell`).
...
| Призматическая | `prism` | — | Ровно один катод + один анод | `аноды = катоды` или `аноды = катоды + 1`; лишний катод запрещён; v1.1 использует pouch-like config |
| Цилиндрическая | `cylindrical` | — | Ровно один катод + один анод | `аноды = катоды` или `аноды = катоды + 1`; лишний катод запрещён |

Отдельные таблицы `battery_coin_config` / `battery_pouch_config` / `battery_cyl_config` хранят параметры, специфичные для форм-фактора. В v1.1 `prism` хранится как отдельный `form_factor`, но временно использует `battery_pouch_config`, пока prism-specific физические параметры не определены лабораторией. Таблица `battery_coin_config` содержит `coin_size_code` (2016 / 2025 / 2032), `coin_layout` (SE / ES / ESE), толщину и количество проставок.
...
Миграции `d031_harden_battery_stack_validate_trigger.sql` и `d036_add_prism_form_factor.sql` обновляют DB-триггер `validate_battery_stack()` под это правило стека. Сервис `batteryElectrodeStackService.js` сохраняет строки в trigger-safe paired order `A1, C1, A2, C2` (анод перед соответствующим катодом), но исходные `position_index` не меняет.
...
На текущий момент (25.05.2026, локальная БД `badb_app_v1`) база данных содержит:
...
| Количество миграций | 49 SQL-файлов в `migrations/` и 49 SQL-файлов в `migrations_ASCII/`: базовый диапазон `001`–`020` плюс Dalia-миграции `d013`–`d040` |
| Авторитетный журнал применённых миграций | `public.schema_migrations` (`dima = 21`, `dalia = 28` после `d040`); flat `migrations_log.txt` — служебные контрольные заметки |
...
### 3.6.2. Поток данных для расчёта плановых количеств пасты

```mermaid
sequenceDiagram
    actor Пользователь
    participant UI as Vanilla Tapes page
    participant Recipe as tape_recipe_lines
    participant Actuals as tape_recipe_line_actuals
    participant Components as material_instance_components

    Пользователь->>UI: Выбирает рецепт, режим расчёта и целевую массу
    UI->>Recipe: Загружает строки рецепта и проценты
    Пользователь->>UI: Выбирает экземпляры материалов
    UI->>Actuals: Сохраняет выбранные material_instance_id
    UI->>Components: Загружает состав экземпляров-смесей
    UI->>UI: Рассчитывает целевую сухую массу строк рецепта
    UI->>UI: Рассчитывает массу экземпляра к взвешиванию
    UI-->>Пользователь: Показывает плановые количества и развёрнутый расчёт
```

Расчёт выполняется по строкам рецепта, участвующим в процентном расчёте.
Активная строка (`cathode_active` или `anode_active`) задаёт связь между
целевой массой активного материала и общей сухой массой смеси. Если выбранный
экземпляр материала сам является смесью, система раскрывает его компонентные
доли и учитывает вклад компонентов при расчёте остальных строк, чтобы избежать
двойного счёта.

Строки, исключённые из процентного расчёта, включая текущую роль растворителя,
заполняются и сохраняются как фактические значения, но не получают
автоматическую плановую сухую массу.

### 3.6.3. Поток данных для расчёта ёмкости партии электродов
...
### 3.6.4. Поток данных для загрузки файла циклирования
...
- Node.js — MIT;
- Текущий vanilla-интерфейс использует HTML/JavaScript/CSS без отдельной runtime-лицензии;
- Vue 3, Vite, Pinia — MIT (для отдельной поверхности следующего интерфейса);
- Express — MIT;
- PrimeVue — MIT (для отдельной поверхности следующего интерфейса);
...
- Документация Vue 3 (https://vuejs.org/) — для синхронизация следующего интерфейса работ.
...
- `BADB_main/docs/archive/superseded/` — исторический контекст происхождения решений; не текущая основа сверки без проверки по коду и каноническим docs.
...
| `schema_migrations` | Авторитетный журнал применённых миграций, созданный и заполненный миграцией `d032_create_schema_migrations_table.sql` |
...
| `tape_step_coating` | Параметры нанесения покрытия (с `coating_sidedness`, зазорами сторон и измеренной толщиной после нанесения/сушки до каландрирования) |
