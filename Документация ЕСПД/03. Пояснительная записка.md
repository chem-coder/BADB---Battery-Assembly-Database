...
На текущий момент (15.05.2026, локальная БД `badb_app_v1`) база данных содержит:
...
| Количество миграций | 46 SQL-файлов в `migrations/` и 46 SQL-файлов в `migrations_ASCII/`: базовый диапазон `001`–`020` плюс Dalia-миграции `d013`–`d037` |
| Authoritative migration ledger | `public.schema_migrations` (`dima = 21`, `dalia = 25` после `d037`); flat `migrations_log.txt` — human checkpoint notes only |
