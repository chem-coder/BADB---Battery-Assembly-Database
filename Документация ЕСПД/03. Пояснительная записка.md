...
| Количество миграций | 44 SQL-файла в `migrations/` и 44 SQL-файла в `migrations_ASCII/`: базовый диапазон `001`–`020` плюс Dalia-миграции `d013`–`d035` |
| Authoritative migration ledger | `public.schema_migrations` (`dima = 21`, `dalia = 23` после `d035`); flat `migrations_log.txt` — human checkpoint notes only |
