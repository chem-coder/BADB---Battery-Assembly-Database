...
| Количество миграций | 45 SQL-файлов в `migrations/` и 45 SQL-файлов в `migrations_ASCII/`: базовый диапазон `001`–`020` плюс Dalia-миграции `d013`–`d036` |
| Authoritative migration ledger | `public.schema_migrations` (`dima = 21`, `dalia = 24` после `d036`); flat `migrations_log.txt` — human checkpoint notes only |
