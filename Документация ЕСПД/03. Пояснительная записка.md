...
| Количество миграций | 41 SQL-файл в `migrations/` и 41 SQL-файл в `migrations_ASCII/`: базовый диапазон `001`–`020` плюс Dalia-миграции `d013`–`d032` |
| Authoritative migration ledger | `public.schema_migrations` (`dima = 21`, `dalia = 20` на текущей локальной БД); flat `migrations_log.txt` — human checkpoint notes only |
